...
import { useState, useRef, useEffect } from "react";
import { usePathname } from "next/navigation"; // 1. Import usePathname for active links
import Image from "next/image";
import Link from "next/link";
import { Search, Menu, X } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20 relative">
                ...
                <Link
                  ...
                  className={`text-sm xl:text-base font-medium whitespace-nowrap transition-colors duration-200 ${
                    isActive
                      ...
                      : "text-gray-700 hover:text-primary" // Inactive Style
                  }`}
                >
                  {link.name}
                </Link>
          ...
          <div className="hidden lg:flex items-center gap-4 z-10 bg-white pl-2">
            ...
            <Link
              href="/booking"
              className="bg-primary text-white px-5 xl:px-6 py-2.5 rounded-lg font-medium hover:bg-primary/90 whitespace-nowrap text-sm xl:text-base"
            >
              Book Now
            </Link>
          </div>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[80%] sm:w-[60%] bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${
             ...
             return (
              <Link
                ...
                className={`block px-4 py-3 rounded-lg text-base font-medium transition-all duration-200 ${
                  isActive
                    ? "bg-primary/10 text-primary translate-x-2" // Active Mobile Style
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50/50 space-y-4">
            <div className="relative w-full">
              <input
                type="text"
                placeholder="Search..."
                className="w-full h-11 border border-gray-200 rounded-lg px-4 text-gray-700 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary placeholder-gray-400 bg-white"
              />
            ...
            <Link
              href="/booking"
              onClick={() => setIsOpen(false)}
              className="w-full flex items-center justify-center py-3 rounded-lg bg-primary text-white font-medium hover:bg-primary/90 transition-colors shadow-md"
            >
              Book Now
            </Link>
        </div>
